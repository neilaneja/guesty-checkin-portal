...
// Scoped to specific listing IDs and a ±48 hour window
// ─────────────────────────────────────────────
async function searchReservations(token, lastName, listingIds) {
  ...
  // If we have specific listing IDs, filter by them
  if (listingIds && listingIds.length > 0) {
    filters.push({ operator: '$in', field: 'listingId', value: listingIds });
  }
